...
const UI = (() => {
    ...
    return `
      <div class="auth-container animate-fade-in">
        <div class="auth-card">
          ...
          <div class="divider" style="margin: 24px 0; display: flex; align-items: center; text-align: center; color: var(--text-muted);">
            <div style="flex: 1; border-bottom: 1px solid var(--border-primary);"></div>
            <span style="padding: 0 10px; font-size: 0.9rem;">OR</span>
            <div style="flex: 1; border-bottom: 1px solid var(--border-primary);"></div>
          </div>
          <div style="margin-bottom: 16px;">
            <button type="button" class="btn btn-block" style="background: white; color: #333; border: 1px solid #ccc; display: flex; align-items: center; justify-content: center; gap: 8px;" onclick="UI.handleFirebaseGoogleLogin()">
          ...
          <p class="auth-footer text-center mt-lg">
  ...
  return {
    ...
    async handleFirebaseGoogleLogin() {
      if (!window.FirebaseAuth || !window.FirebaseProvider) {
        showToast('Firebase not loaded yet.', 'warning');
        return;
      }
      try {
